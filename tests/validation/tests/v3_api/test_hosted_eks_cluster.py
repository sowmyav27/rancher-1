...
@ekscredential
def test_eks_v2_hosted_eks_cluster_edit():
    cluster_name = random_test_name("test-auto-eks")
    eks_config_temp = get_eks_config_basic(cluster_name)
    cluster_config = {
        "eksConfig": eks_config_temp,
...
@ekscredential
def test_eks_v2_hosted_eks_cluster_delete():
    cluster_name = random_test_name("test-auto-eks")
    eks_config_temp = get_eks_config_basic(cluster_name)
    cluster_config = {
        "eksConfig": eks_config_temp,
    ...
    wait_for_cluster_delete_eks(cluster_name)
...
def validate_nodegroup(nodegroup_list, cluster_name):
    for nodegroup in nodegroup_list:
        ...
        # k8s version check
        eks_cluster = AmazonWebServices().describe_eks_cluster(cluster_name)
        assert eks_cluster["cluster"]["version"] == eks_nodegroup["version"], \
            "Mismatch between K8s version of cluster and nodegroup"

        # status of nodegroup
        assert eks_nodegroup["status"] == "ACTIVE", \
            "Nodegroups are not in active status"

        # check scalingConfig
        assert nodegroup["maxSize"] == eks_nodegroup["scalingConfig"]["maxSize"], \
            "maxSize is incorrect on the nodes"
        assert nodegroup["minSize"] == eks_nodegroup["scalingConfig"]["minSize"], \
            "minSize is incorrect on the nodes"
        assert nodegroup["minSize"] == eks_nodegroup["scalingConfig"]["minSize"], \
            "minSize is incorrect on the nodes"

        # check instance type
        assert nodegroup["instanceType"] == eks_nodegroup["instanceTypes"][0], \
            "instanceType is incorrect on the nodes"

        # check disk size
        assert nodegroup["diskSize"] == eks_nodegroup["diskSize"], \
            "diskSize is incorrect on the nodes"
        # check ec2SshKey
        if nodegroup["ec2SshKey"] is not None:
            assert nodegroup["ec2SshKey"] == eks_nodegroup["ec2SshKey"], \
                "Ssh key is incorrect on the nodes"


def wait_for_cluster_delete_eks(cluster_name, timeout=DEFAULT_TIMEOUT_EKS):
    start = time.time()
